...
class CustomerViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request):
        ...
        customers = self.get_queryset().values_list('name', flat=True)
...
class TrackViewSet(viewsets.ModelViewSet):
    ...
    def swap(self, request, pk=None):
         if 'track1Id' in request.data and 'track2Id' in request.data:
            ...
            track1.plates.update(is_manual=True)
            track2.plates.update(is_manual=True)
...
class ResetCalculationView(APIView):
    """
    API endpoint for resetting plates and starting a calculation
    """
    permission_classes = [permissions.AllowAny]

    def post(self, request):
        """Reset plates and start a calculation"""
        from .tasks import calculate_task
        from .models import Plate

        # Reset all plates (set is_manual and is_deleted to False)
        Plate.all_objects.update(is_manual=False, is_deleted=False)
        ...
        return Response({
            'status': 'success', 
            'message': 'Plates reset and calculation started successfully',
            'task_id': task.id
        })
...
class AlgorithmDemoView(APIView):
    ...
    """
    def post(self, request):
        """Process uploaded file and start algorithm demonstration task"""
        ...
        try:
            ...
            from .tasks import calculate_algorithm_demo

            # Parse the uploaded JSON file
            file_content = demo_file.read().decode('utf-8')

            # Start the calculation task asynchronously
            task = calculate_algorithm_demo.delay(file_content)

            # Return the task ID to the client
            return Response({
                'status': 'success', 
                'message': 'Демонстрация алгоритма запущена успешно',
                'task_id': task.id
            })
        except Exception as e:
            import traceback
    ...
    def get(self, request, task_id=None):
        """Get the result of a specific algorithm demo task"""
        if not task_id:
        ...
        try:
            ...
            calculation_status = CalculationStatus.objects.get(task_id=task_id)

            # If the calculation is not completed, return the status
            if calculation_status.status != 'completed':
                return Response({
                    'status': calculation_status.status,
                    'progress': calculation_status.progress,
                    'message': calculation_status.message
                })

            # If the calculation is completed, try to get the result from the task result
            ...
            if result.successful():
                ...
                if 'result' in task_result:
                    return Response({'result': task_result['result'], "status": "completed"})

            # If we couldn't get the result, return an error
            return Response({'error': 'Result not available'}, status=status.HTTP_404_NOT_FOUND)

        except CalculationStatus.DoesNotExist:
            return Response({'error': f'No calculation found with task_id {task_id}'}, status=status.HTTP_404_NOT_FOUND)
        except Exception as e:
